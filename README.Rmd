...
More information on the package can be found via the function mechanics [Introduction to the mcst package](https://github.com/calebong/mcst/blob/main/inst/doc/portfolio_risk_simulation_vignette.pdf),usage illustration [Sample Scenario Illustrations](https://github.com/calebong/mcst/blob/main/inst/doc/portfolio_risk_simulation_scenarios.pdf), and reference manual [Reference Manual](https://github.com/calebong/mcst/blob/main/mcst_0.1.0.pdf) documentations.
...
returns_data=returns_data %>% dplyr::select(week, NVDA, JPM, XOM)
```
...
We can also inspect the portfolio risk contributions under non-stressed and stressed scenarios:

```{r s5-run the decomposition, cache=TRUE, results="hide", warning=FALSE, message=FALSE}
decomp_result_stag <- decompose_portfolio_risk(result_stag, 
                                            var_method = "tail")

```

```{r show decomposition, echo = FALSE, results = 'hide'}
# Save plot to file
decomp_result_stag_summary_canvas <- decomp_result_stag$plots$summary_canvas
decomp_result_stag_risk_decomposition <- decomp_result_stag$plots$risk_decomposition
```

```{r save decomposition, include=FALSE}
# Save plot to file
ggsave("man/figures/decomp_result_stag_risk_decomposition.png", decomp_result_stag_risk_decomposition , width = 10, height = 7, dpi = 150)
ggsave("man/figures/decomp_result_stag_summary_canvas.png", decomp_result_stag_summary_canvas, width = 10, height = 7, dpi = 150)
```
